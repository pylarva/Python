...
class Asset(BaseServiceList):
    def __init__(self):
        condition_config = [
            ...
            {'name': 'business_3', 'text': '业务类型1', 'condition_type': 'select', 'global_name': 'business_3_list'},
        ]
        table_config = [
            {
                'q': 'id',
    ...
    def fetch_assets(self, request):
        ...
        try:
            ...
            asset_count = models.BusinessThree.objects.filter(conditions).count()
            page_info = PageInfo(request.GET.get('pager', None), asset_count)
            asset_list = models.BusinessThree.objects.filter(conditions).extra(select=self.extra_select).values(
    ...
    def delete_assets(request):
        ...
        try:
            ...
            models.BusinessThree.objects.filter(id__in=id_list).delete()
    ...
    def put_assets(request):
        ...
        try:
            ...
            for row_dict in update_list:
                ...
                try:
                    models.BusinessThree.objects.filter(id=nid).update(**row_dict)
                except Exception as e:
