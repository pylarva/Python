            ...
            while True:
                ...
                if selected_host_group:
                    ...
                    while True:
                        ...
                        if choice.isdigit():
                            choice = int(choice)
                            if 0 <= choice <= index:  # 合法选项
                                print("going to logon ....", selected_host_group.bind_hosts.all()[choice])
                                # print(choice, bind_host.remote_user.password, bind_host.remote_user.username, bind_host.host.ip_addr)
                                # bind_host = selected_host_group.bind_hosts.all()[choice]
                                # subprocess.run('sshpass -p %s ssh %s@%s' % (bind_host.remote_user.password,
                                #                                             bind_host.remote_user.username,
                                #                                             bind_host.host.ip_addr), shell=True)
                                break
                                bind_host = selected_host_group.bind_hosts.all()[choice]
                                ssh_tag = uuid.uuid4()
