...
function CreateSelect(attrs,csses,option_dict,item_key,item_value,current_val){
	...
	$.each(option_dict,function(k,v){
		...
		if(sel_value==current_val){
			$(opt1).text(sel_text).attr('value', sel_value).attr('text', sel_text).attr()
		}else{
			$(opt1).text(sel_text).attr('value', sel_value).attr('text', sel_text).appendTo()
		}
	});
	return sel;
}
	...
	$tr.children().each(function(){
		if($(this).attr('edit') == "true"){
			if($(this).attr('edit-type') == "select"){
				...
				var selelct_tag = CreateSelect({"onchange": "MultiSelect(this);"}, {}, window[global_key])
